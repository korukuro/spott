...
export default function AIEventCreator({ onEventGenerated }) {
  ...
  const generateEvent = async () => {
    ...
    try {
      ...
      // 🚨 THE FIX: Manually check for HTTP errors!
      if (!response.ok) {
        // Throw the specific error message sent from the backend
        throw new Error(data.error || "Failed to generate event");
      }

      // If we make it here, the request was a 100% success
      onEventGenerated(data);
      toast.success("Event details generated! Review and customize below.");
      setIsOpen(false);
      setPrompt("");
    } catch (error) {
      // Now, 429 rate limits and 500 server errors will properly end up here!
