...
const SearchLocationBar = () => {
    ...
    const {data: searchResults, isLoading: searchLoading} = useConvexQuery(api.search.searchEvent, searchQuery?.trim().length >= 2 ? {query : searchQuery, limit: 5} : 'skip')
    ...
    const handleLocationSelect = async (city, state) => {
        try {
            ...
            console.error("Could update location:",error);
        }
    }
